...
// PERBAIKAN: Gunakan kolom pin_validasi (bukan pin)
$guru_list = fetchAll("SELECT id_guru, nama_guru, nip, pin_validasi, status FROM tb_guru ORDER BY nama_guru");
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="p-4 font-medium text-gray-600"><?= $guru['nip'] ?? '-' ?></td>
                                <td class="p-4 font-bold text-navy"><?= htmlspecialchars($guru['nama_guru']) ?></td>
                                <td class="p-4">
                                    <code class="bg-gray-100 px-3 py-1 rounded font-mono text-sm"><?= $guru['pin_validasi'] ?></code>
                                </td>
                                <td class="p-4">
                                ...
                                <td class="p-4">
                                    <div class="flex space-x-2">
                                        <button onclick="editGuru(<?= $guru['id_guru'] ?>, '<?= htmlspecialchars($guru['nama_guru']) ?>', '<?= $guru['nip'] ?>', '<?= $guru['pin_validasi'] ?>')" 
    ...
    <div class="bg-white rounded-lg max-w-md w-full">
        ...
        <form action="../../actions/tambah_guru.php" method="POST" class="p-6 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="pin_validasi" maxlength="6" pattern="[0-9]{6}" required 
...
<div id="modal-edit" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-lg max-w-md w-full">
        ...
        <form action="../../actions/edit_guru.php" method="POST" class="p-6 space-y-4">
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">PIN (6 Digit) *</label>
                <input type="text" name="pin_validasi" id="edit-pin-validasi" maxlength="6" pattern="[0-9]{6}" required 
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-navy">
            </div>
            <div class="flex space-x-2">
...
<script>
function editGuru(id, nama, nip, pin) {
    ...
    document.getElementById('edit-pin-validasi').value = pin;
