 ...
 * Menghapus transaksi pelanggaran dan rollback poin
 */
...
try {
    ...
    // 1. Ambil data transaksi untuk rollback poin
    $transaksi = fetchOne("
        SELECT h.id_anggota, SUM(d.poin_saat_itu) as total_poin, jp.id_kategori
    ...
    // 2. Ambil detail poin per kategori
    $details = fetchAll("
        SELECT jp.id_kategori, SUM(d.poin_saat_itu) as poin
    ...
    foreach ($details as $detail) {
        ...
        if ($kategori == 1) {
            executeQuery("UPDATE tb_anggota_kelas SET poin_kelakuan = GREATEST(0, poin_kelakuan - :poin), total_poin_umum = GREATEST(0, total_poin_umum - :poin) WHERE id_anggota = :id", 
                ['poin' => $poin, 'id' => $transaksi['id_anggota']]);
        } elseif ($kategori == 2) {
    ...
    // 4. Hapus detail, sanksi, dan header (CASCADE akan handle detail & sanksi)
